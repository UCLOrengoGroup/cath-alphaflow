...
def measure_globularity_af_cif(
    af_domain_list,
    af_chain_mmcif_dir,
    af_domain_globularity,
    distance_cutoff,
    ...
    for af_domain_id in af_domain_list_reader:
        ...
        LOG.debug(
            f"Processed entry: {af_domain_id}\tpacking_density:{af_domain_packing_density}\tgiration:{af_domain_normed_radius_gyration}"
        )
    ...
    *,
    ...
    with open_func(f"{cif_path}", mode="rt") as cif_fh:
        ...
        for residue_number in domain_residues:
            ...
            if (
                residue.get_resname() in hydrophobic_list
                and not residue.is_disordered()
            ):
